...
void Snake::Render() {
	...
	sf::RenderWindow gameWindow(sf::VideoMode(w, h + 2 * height), "Snake - The Game", sf::Style::Close | sf::Style::Titlebar);
...
void Snake::DisplayScore(sf::RenderWindow& gameWindow) {
	...
	sf::RectangleShape background(sf::Vector2f(width*picSize+1, height * 2+1));
...
void Snake::GameOver(sf::RenderWindow& gameWindow) {
	std::cout << "YOU LOSE";
	scoreString += std::to_string(score)+"AAAAAAAAA";
	sf::RectangleShape shape(sf::Vector2f(width * picSize * 5, height * picSize * 5));
